...
import { 
  ...
  Image as ImageIcon, Video, Calendar, Briefcase, Sparkles,
  TrendingUp, Users, Award, Building2, ChevronRight
} from 'lucide-react';
...
export default function SocialFeedPage() {
  ...
  const fallbackPosts: Post[] = [
    {
      ...
      trustLevel: 'verified',
      content: 'Today we gathered at Uluru for the annual Tjukurpa ceremony. Watching our young ones learn the ancient stories fills my heart with hope. Remember: your culture is your strength on any career path. 🌏✨',
  ...
  const fetchPosts = useCallback(async (pageNum = 1, append = false) => {
    ...
    try {
      const endpoint = `/social-feed?page=${pageNum}&limit=10`;
      const res = await api(endpoint);

      if (!res.ok) throw new Error('Failed to fetch');

      const data: any = res.data;
      const apiPosts = (data.posts || data || []).map((p: any) => ({
        id: p.id,
        authorName: p.authorName || 'Community Member',
        authorAvatar: p.authorAvatar || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop',
        authorTitle: p.authorTitle || '',
        trustLevel: p.trustLevel || 'basic',
        content: p.content,
        mediaUrl: (() => {
          if (typeof p.mediaUrl === 'string' && p.mediaUrl.length > 0) return p.mediaUrl;
          if (Array.isArray(p.mediaUrls) && typeof p.mediaUrls[0] === 'string') return p.mediaUrls[0];
  ...
  const handleReaction = async (postId: string, type: string) => {
    ...
    if (!res.ok) {
      ...
      setPosts(prev => prev.map(p => {
        if (p.id !== postId) return p;
        return { ...p, reactions: { ...p.reactions, [type]: Math.max(0, (p.reactions[type] || 0) - 1) } };
      }));
    }
  };
  ...
  return (
    <div className={`${spaceGrotesk.className} min-h-screen`} style={{ 
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="hidden lg:block lg:col-span-3">
            <div className="sticky top-20 space-y-4">
              ...
              <div className="rounded-xl p-4 bg-white/5 backdrop-blur-md border border-white/10">
                <h3 className="text-sm font-semibold mb-3 text-emerald-400">Quick Actions</h3>
                <div className="space-y-1">
                  <Link href="/social-feed/new" className="flex items-center gap-2 p-2 rounded-lg text-gray-300 hover:bg-white/5 transition-colors">
                    <Sparkles className="w-4 h-4 text-amber-400" />
                    Create Post
                  </Link>
                  <Link href="/jobs" className="flex items-center gap-2 p-2 rounded-lg text-gray-300 hover:bg-white/5 transition-colors">
                    <Briefcase className="w-4 h-4 text-blue-400" />
                    Browse Jobs
                  </Link>
                  <Link href="/events" className="flex items-center gap-2 p-2 rounded-lg text-gray-300 hover:bg-white/5 transition-colors">
                    <Calendar className="w-4 h-4 text-pink-400" />
                    Upcoming Events
                  </Link>
                  <Link href="/mentorship" className="flex items-center gap-2 p-2 rounded-lg text-gray-300 hover:bg-white/5 transition-colors">
              ...
              {/* Trending Topics */}
              <div className="rounded-xl p-4 bg-white/5 backdrop-blur-md border border-white/10">
                <h3 className="text-sm font-semibold mb-3 text-amber-400">Trending Topics</h3>
                <div className="space-y-2">
                  {trendingTopics.map((topic, i) => (
                    ...
                    >
                      <span className="text-sky-400 text-sm">{topic.tag}</span>
          ...
          <main className="lg:col-span-6">
            ...
            <div className="mb-6 -mx-4 px-4 overflow-x-auto scrollbar-hide">
              <div className="flex gap-3 pb-2">
                ...
                <Link href="/social-feed/new" className="flex flex-col items-center gap-1 shrink-0">
                  <div className="relative w-16 h-16 rounded-full bg-gradient-to-br from-brand-500/30 to-purple-600/30 flex items-center justify-center border-2 border-dashed border-white/30">
                ...
                {/* Sample Stories */}
                {[
                  { name: 'Sarah', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop', hasNew: true },
                  { name: 'Jarrah', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop', hasNew: true },
                  { name: 'BHP', avatar: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=100&h=100&fit=crop', hasNew: true },
                  { name: 'Chelsea', avatar: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=100&h=100&fit=crop', hasNew: false },
                  { name: 'Deadly Sci', avatar: 'https://images.unsplash.com/photo-1532094349884-543bc11b234d?w=100&h=100&fit=crop', hasNew: true },
                  { name: 'Emma', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&h=100&fit=crop', hasNew: false },
                ].map((story, i) => (
                  <button key={i} className="flex flex-col items-center gap-1 shrink-0">
                    <div className={`relative w-16 h-16 rounded-full p-0.5 ${story.hasNew ? 'bg-gradient-to-r from-amber-500 via-pink-500 to-purple-500' : 'bg-white/20'}`}>
                      <Image
                        src={story.avatar}
                        alt={story.name}
                        width={64}
                        ...
                        className="w-full h-full rounded-full object-cover border-2 border-[#1A0F2E]"
                      />
                    </div>
                    <span className="text-[10px] text-gray-400 truncate w-16 text-center">{story.name}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Create Post Card */}
            <div className="rounded-xl p-4 mb-6 bg-white/5 backdrop-blur-md border border-white/10">
              <div className="flex gap-3">
                ...
                <Link 
                  ...
                  className="flex-1 p-3 rounded-lg bg-white/5 text-gray-400 hover:bg-white/10 transition-colors text-left"
                >
                  Share your story with the community...
                </Link>
              </div>
              <div className="flex gap-4 mt-3 pt-3 border-t border-white/10">
                <button className="text-sm flex items-center gap-1 text-emerald-400 hover:text-emerald-300">
                  <ImageIcon className="w-4 h-4" /> Photo
                </button>
                <button className="text-sm flex items-center gap-1 text-sky-400 hover:text-sky-300">
                  <Video className="w-4 h-4" /> Video
                </button>
                <button className="text-sm flex items-center gap-1 text-pink-400 hover:text-pink-300">
                  <Calendar className="w-4 h-4" /> Event
                </button>
                <button className="text-sm flex items-center gap-1 text-amber-400 hover:text-amber-300">
              ...
              <div className="space-y-6">
                {posts.map((post, index) => (
                  ...
                  >
                    {/* Sponsored Label */}
                    {post.isSponsored && (
                      <div className="px-4 py-2 text-xs font-medium text-amber-400 border-b border-amber-500/20 flex items-center gap-2">
          ...
          <aside className="hidden lg:block lg:col-span-3">
            <div className="sticky top-20 space-y-4">
              ...
              <div className="rounded-xl p-4 bg-white/5 backdrop-blur-md border border-amber-500/30">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-amber-400">Partner Opportunities</h3>
                  <span className="text-[10px] px-2 py-0.5 rounded bg-amber-500/20 text-amber-400">AD</span>
                </div>
                <div className="space-y-2">
                ...
                <Link 
                  href="/jobs" 
                  className="flex items-center justify-center gap-1 mt-3 py-2 rounded-lg text-sm font-medium border border-amber-500/40 text-amber-400 hover:bg-amber-500/10 transition-colors"
                >
              ...
              {/* Community Stats */}
              <div className="rounded-xl p-4 bg-white/5 backdrop-blur-md border border-white/10">
                <h3 className="text-sm font-semibold mb-3 text-sky-400 flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-3 text-center">
                  ...
                  <div className="bg-white/5 rounded-lg p-2">
                    <div className="text-lg font-bold text-emerald-400">8.2K</div>
                  ...
                  <div className="bg-white/5 rounded-lg p-2">
                    <div className="text-lg font-bold text-sky-400">250+</div>
              ...
              <div className="text-center py-3">
                <p className="text-[10px] text-gray-500">
                  <span className="text-amber-400">✦</span> We acknowledge Traditional Custodians <span className="text-emerald-400">✦</span>
                </p>
              </div>
            </div>
          </aside>
      ...
      <nav className="fixed bottom-0 left-0 right-0 lg:hidden bg-gradient-to-t from-[#1A0F2E] to-[#1A0F2E]/95 backdrop-blur-lg border-t border-white/10 z-50">
        <div className="flex justify-around items-center py-2 px-4">
          <Link href="/" className="flex flex-col items-center gap-1 p-2 text-gray-400 hover:text-white transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className="text-[10px]">Home</span>
          </Link>
          <Link href="/social-feed" className="flex flex-col items-center gap-1 p-2 text-amber-400 transition-colors">
            <Sparkles className="w-6 h-6" />
          ...
          <Link href="/jobs" className="flex flex-col items-center gap-1 p-2 text-gray-400 hover:text-white transition-colors">
            <Briefcase className="w-6 h-6" />
            <span className="text-[10px]">Jobs</span>
          </Link>
          <Link href="/profile" className="flex flex-col items-center gap-1 p-2 text-gray-400 hover:text-white transition-colors">
