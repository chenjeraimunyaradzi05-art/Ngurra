...
export default function FeedPage() {
  ...
  const fetchPosts = useCallback(async (pageNum = 1, append = false) => {
    ...
    try {
      ...
      const endpoint = activeTab === 'for-you' || activeTab === 'following' 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="hidden lg:block lg:col-span-3">
            ...
            >
              {/* Quick Actions */}
              <div>
                <h3 className="text-sm font-semibold mb-3" style={{ color: accentPink }}>Quick Actions</h3>
                <div className="space-y-2">
                  <Link href="/feed/create" className="flex items-center gap-2 p-2 rounded-lg transition-colors text-slate-700 hover:bg-pink-50 hover:text-pink-600">
                    ✏️ Create Post
                  </Link>
                  <Link href="/jobs" className="flex items-center gap-2 p-2 rounded-lg transition-colors text-slate-700 hover:bg-pink-50 hover:text-pink-600">
                    💼 Browse Jobs
                  </Link>
                  <Link href="/events" className="flex items-center gap-2 p-2 rounded-lg transition-colors text-slate-700 hover:bg-pink-50 hover:text-pink-600">
                    📅 Upcoming Events
                  </Link>
                  <Link href="/mentorship" className="flex items-center gap-2 p-2 rounded-lg transition-colors text-slate-700 hover:bg-pink-50 hover:text-pink-600">
                    🤝 Find a Mentor
                  </Link>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                <h3 className="text-sm font-semibold mb-3" style={{ color: accentPurple }}>Trending Topics</h3>
                <div className="space-y-2">
                  {trendingTopics.map((topic, i) => (
                    <Link key={i} href={`/search?q=${topic.tag}`} className="flex items-center justify-between p-2 rounded-lg transition-colors text-slate-700 hover:bg-purple-50">
          ...
          <main className="lg:col-span-6">
            ...
            >
              ...
              <div className="flex gap-4 mt-3 pt-3 border-t border-slate-100">
                <button className="text-sm flex items-center gap-1" style={{ color: accentPink }}>📷 Photo</button>
                <button className="text-sm flex items-center gap-1" style={{ color: accentPurple }}>🎥 Video</button>
                <button className="text-sm flex items-center gap-1" style={{ color: accentPink }}>📅 Event</button>
                <button className="text-sm flex items-center gap-1" style={{ color: accentPurple }}>💼 Job</button>
              </div>
            </div>
              ...
              <div className="space-y-6">
                {posts.map((post, index) => (
                  ...
                  >
                    {/* Sponsored Label */}
                    {post.isSponsored && (
                      <div className="px-4 py-2 text-xs font-medium flex items-center gap-2 border-b border-slate-100" style={{ color: accentPink }}>
                    ...
                    <div className="p-4 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {post.isOrganization && (
                            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(139, 92, 246, 0.15)', color: accentPurple }}>Organization</span>
                          )}
                        </div>
                    ...
                    <div className="px-4 pb-4">
                      <p className="leading-relaxed whitespace-pre-line text-slate-700">{post.content}</p>
                    </div>
                    ...
                    <div className="px-4 py-3 flex items-center justify-between text-sm border-t border-slate-100 text-slate-500">
                      <div className="flex items-center gap-1">
                        {Object.entries(post.reactions).slice(0, 3).map(([type]) => (
            ...
            <div className="sticky top-20 space-y-4">
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold" style={{ color: accentPink }}>Partner Opportunities</h3>
                  <span className="text-[10px] px-2 py-0.5 rounded" style={{ background: 'rgba(233, 30, 140, 0.15)', color: accentPink }}>AD</span>
                </div>
                <div className="space-y-3">
                  {partners.map((partner, i) => (
                    <Link key={i} href="/jobs" className="flex items-center justify-between p-2 rounded-lg transition-colors hover:bg-pink-50">
                      <div className="flex items-center gap-2">
                        <span className="text-xl">{partner.logo}</span>
                        <span className="text-slate-700">{partner.name}</span>
                      </div>
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(139, 92, 246, 0.15)', color: accentPurple }}>{partner.jobs} jobs</span>
                    </Link>
                  ))}
                </div>
                <Link 
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] px-2 py-0.5 rounded" style={{ background: 'rgba(139, 92, 246, 0.15)', color: accentPurple }}>SPONSORED</span>
                </div>
                <h3 className="font-semibold mb-2" style={{ color: accentPurple }}>Free TAFE Courses</h3>
                <p className="text-sm mb-3 text-slate-500">
                  Fully funded Certificate III & IV programs for First Nations students in Health, IT, and Construction.
                </p>
                <Link 
                  href="/courses"
              ...
              <div 
                ...
                style={{ boxShadow: '0 4px 20px rgba(15, 23, 42, 0.06)' }}
              >
                <h3 className="text-sm font-semibold mb-3" style={{ color: accentPink }}>Community Impact</h3>
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div>
                    <div className="text-xl font-bold" style={{ color: accentPink }}>12.5K</div>
                    <div className="text-[10px] text-slate-400">Jobs Filled</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold" style={{ color: accentPurple }}>8.2K</div>
                    <div className="text-[10px] text-slate-400">Mentorships</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold" style={{ color: accentPink }}>$45M</div>
                    <div className="text-[10px] text-slate-400">Wages Earned</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold" style={{ color: accentPurple }}>250+</div>
              ...
              <div className="text-center py-3">
                <p className="text-[10px] text-slate-400">
                  <span style={{ color: accentPink }}>✦</span> We acknowledge Traditional Custodians <span style={{ color: accentPurple }}>✦</span>
                </p>
              </div>
            </div>
          </aside>
