...
export default function MessagesPage() {
        ...
        prev.map(conv => 
          conv.id === message.conversationId 
            ? { 
                ...
                unreadCount: message.senderId !== myUserId ? (conv.unreadCount || 0) + 1 : conv.unreadCount
              }
            : conv
        )
      );
  ...
  const mockMessages = [
    ...
    },
    {
      id: '3',
      senderId: 'other',
      content: 'We\'re looking for someone with your skills in community engagement. The role involves working closely with First Nations communities.',
      timestamp: '10:35 AM',
      isRead: true
    },
    {
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1">
                        <span className="font-medium text-slate-800 truncate">{convo.participant.name}</span>
                        {convo.participant.isVerified && (
                          <span className="text-xs" style={{ color: accentPink }}>{getTrustBadge(convo.participant.trustLevel).icon}</span>
                        )}
                      </div>
              ...
              <header className="p-4 border-b border-slate-200 flex items-center gap-3 bg-white">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h2 className="font-semibold text-slate-800">{selectedConversation.participant.name}</h2>
                    {selectedConversation.participant.isVerified && (
                      <span className="text-sm" style={{ color: accentPink }}>{getTrustBadge(selectedConversation.participant.trustLevel).icon}</span>
                    )}
                  </div>
                ...
                <div className="flex gap-2">
                  <button className="p-2 rounded-full hover:bg-slate-100 transition-colors" title="Video Call">
                    📹
                  </button>
                  <button className="p-2 rounded-full hover:bg-slate-100 transition-colors" title="More Options">
                    ⋯
                  </button>
                </div>
              </header>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                ...
                <div className="mx-auto max-w-md p-3 rounded-lg bg-pink-50 border border-pink-200 text-center">
                  <p className="text-sm text-slate-600">
                    ...
                    <Link href="/settings/safety" className="hover:underline ml-1" style={{ color: accentPink }}>Adjust settings</Link>
                  </p>
                </div>
                ...
                {displayMessages.map(message => (
                  ...
                  >
                    <div 
                      ...
                      style={message.senderId === 'me' ? { background: `linear-gradient(135deg, ${accentPink} 0%, ${accentPurple} 100%)` } : {}}
                    >
